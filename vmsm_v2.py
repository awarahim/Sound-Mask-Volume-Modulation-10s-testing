...
CHANNELS = 1                 # if use 2 channels, the buffer will have 
RATE = 44100                # 44.1kHz
...
def rms(data):
    ...
    return abs(int(rms))  # why absolute value?
...
#             tic = time.time()
            data = rms(in_data)
#             toc = time.time()-tic
#             print('time to calculate rms:', toc)
            
#             tic2 = time.time()
            with open(ref_fname, 'a') as f:
                csvwriter = csv.writer(f)
                csvwriter.writerows([datetime.now().strftime('%H:%M:%S:%f'), str(data)])
#             toc2 = time.time()-tic2
#             print('time to write:', toc2)
...
#             tic2 = time.time()
            with open(err_fname, 'a') as w:
                ...
                csvwriter2.writerows([datetime.now().strftime('%H:%M:%S:%f'), str(data2)])
#             toc2 = time.time() - tic2
#             print('time to write:',toc2)
...
#     writer.writerow(['time',datetime.now().strftime('%H:%M:%S:%f') ,'difference:', difference, 'new volume:', new_vol, 'step size', nu])
#     file.close()
#     
    return new_vol
################################################################################################################################################    
...
# NONBLOCKING MODE    
def whitenoise(volume, stop_event):
...
#          volume_value.value = (volume_value.value + 1) % 100
         ...
         data = set_volume(data,volume) # if something wrong: volume
...
#     logging.info('Start ' + str(datetime.now()))
    ...
    p2 = mp.Process(target=whitenoise, args=(volume_value.value, stop_event))
